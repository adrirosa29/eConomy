...
export default function App() {
  ...
  const subTrend = useMemo(() => {
    ...
    return catYear === CURRENT_YEAR ? rows.slice(0, CURRENT_MONTH_INDEX + 1) : rows;
  }, [transactions, catYear, activeCatName, subSelected]);
  ...
  const sharedStyle = (
    <style>{`
      ...
      .cat-row .chevron { color: ${PALETTE.inkSoft}; transition: color 0.12s ease, transform 0.12s ease; }
      .cat-row:hover .chevron { color: ${PALETTE.gold}; }
      .cat-row.selected { background: ${PALETTE.paperDim}; }
      .cat-row.selected .chevron { color: ${PALETTE.gold}; transform: rotate(90deg); }
      .chip-x {
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  {catBreakdown.map((c) => (
                    <div key={c.category} className={"cat-row" + (activeCatName === c.category ? " selected" : "")} onClick={() => { setCatSelected(c.category); setSubSelected(null); setSubExpanded(false); }} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <span className="chevron" style={{ fontSize: 14, width: 12, textAlign: "center", display: "inline-block" }}>{"\u203A"}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 28px", marginTop: 20 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  {visibleSubBreakdown.map((s) => (
                    ...
                    >
                      <span className="cat-name" style={{ fontSize: 13, width: 140, color: subSelected === s.subcategory ? PALETTE.gold : PALETTE.ink, fontWeight: subSelected === s.subcategory ? 500 : 400, flexShrink: 0 }}>{s.subcategory}</span>
                      <div style={{ flex: 1, height: 8, background: PALETTE.paperDim, borderRadius: 2, overflow: "hidden" }}>
