...
function niceTicks(minValue, maxValue, desiredCount = 5) {
  ...
  let step = Math.ceil(range / (desiredCount - 1) / 100) * 100;
  if (step <= 0) step = 100;
  const start = Math.floor(lo / step) * step;
  const ticks = [];
  for (let v = start; v <= hi + step / 2; v += step) ticks.push(v);
  return ticks;
}
          ...
          <>
            <section style={{ background: PALETTE.coverSoft, borderRadius: 4, padding: "16px 24px", marginBottom: 20, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
              <div>
                ...
                <p style={{ fontSize: 12, color: "rgba(242,244,245,0.72)", margin: 0 }}>{importMsg || "Sube un archivo .json exportado desde Excel para añadirlo a tu registro."}</p>
              </div>
          ...
          <>
            ...
            <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 14, marginBottom: 20 }}>
              <div style={{ background: PALETTE.paper, borderRadius: 4, padding: "16px 18px" }}>
                ...
                <p style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 20, color: PALETTE.income, margin: 0 }}>{formatMoneyRound(resumenTotals.income)}</p>
              </div>
              <div style={{ background: PALETTE.paper, borderRadius: 4, padding: "16px 18px" }}>
                <p style={{ fontSize: 12, color: PALETTE.inkSoft, margin: "0 0 6px" }}>Gastos</p>
                <p style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 20, color: PALETTE.expense, margin: 0 }}>{formatMoneyRound(resumenTotals.expense)}</p>
              </div>
              <div style={{ background: PALETTE.paper, borderRadius: 4, padding: "16px 18px" }}>
                <p style={{ fontSize: 12, color: PALETTE.inkSoft, margin: "0 0 6px" }}>Ahorro</p>
                <p style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 20, color: resumenTotals.savings >= 0 ? PALETTE.income : PALETTE.expense, margin: 0 }}>{formatMoneyRound(resumenTotals.savings)}</p>
              </div>
              <div style={{ background: PALETTE.paper, borderRadius: 4, padding: "16px 18px" }}>
                ...
                <div style={{ width: "100%", height: 260 }}>
                  <ResponsiveContainer>
                    <BarChart data={resumenRows} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      ...
                      <YAxis domain={[flowTicks[0], flowTicks[flowTicks.length - 1]]} ticks={flowTicks} allowDecimals={false} tick={{ fontSize: 10, fill: PALETTE.inkSoft, fontFamily: "IBM Plex Mono" }} axisLine={false} tickLine={false} tickFormatter={(v) => formatMoneyAxis(v)} width={52} />
                ...
                <div style={{ width: "100%", height: 220 }}>
                  <ResponsiveContainer>
                    <BarChart data={resumenRows} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      <CartesianGrid stroke={PALETTE.rule} vertical={false} />
                      <XAxis dataKey="label" interval={0} tick={{ fontSize: 11, fill: PALETTE.inkSoft, fontFamily: "IBM Plex Sans" }} axisLine={{ stroke: PALETTE.rule }} tickLine={false} />
                      <YAxis domain={[savingsTicks[0], savingsTicks[savingsTicks.length - 1]]} ticks={savingsTicks} allowDecimals={false} tick={{ fontSize: 10, fill: PALETTE.inkSoft, fontFamily: "IBM Plex Mono" }} axisLine={false} tickLine={false} tickFormatter={(v) => formatMoneyAxis(v)} width={52} />
                      <Tooltip contentStyle={tooltipStyle} labelStyle={{ color: PALETTE.ink }} itemStyle={{ color: PALETTE.ink }} formatter={(v) => formatMoneyRound(v)} />
          ...
          <>
            ...
            <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 32px", marginBottom: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 4 }}>
                ...
                <p style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 15, color: PALETTE.ink, margin: 0 }}>{formatMoneyRound(catTotal)}</p>
              </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  {catBreakdown.map((c) => (
                    <div key={c.category} className={"cat-row" + (activeCatName === c.category ? " selected" : "")} onClick={() => { setCatSelected(c.category); setSubSelected(null); setSubExpanded(false); }} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 13, width: 85, textAlign: "right", color: PALETTE.inkSoft }}>{formatMoneyRound(c.total)}</span>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div style={{ width: "100%", height: 200 }}>
                  <ResponsiveContainer>
                    <LineChart data={catTrend} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      <CartesianGrid stroke={PALETTE.rule} vertical={false} />
                      <XAxis dataKey="label" interval={0} tick={{ fontSize: 11, fill: PALETTE.inkSoft, fontFamily: "IBM Plex Sans" }} axisLine={{ stroke: PALETTE.rule }} tickLine={false} />
                      <YAxis domain={[catTrendTicks[0], catTrendTicks[catTrendTicks.length - 1]]} ticks={catTrendTicks} allowDecimals={false} tick={{ fontSize: 10, fill: PALETTE.inkSoft, fontFamily: "IBM Plex Mono" }} axisLine={false} tickLine={false} tickFormatter={(v) => formatMoneyAxis(v)} width={52} />
                      <Tooltip contentStyle={tooltipStyle} labelStyle={{ color: PALETTE.ink }} itemStyle={{ color: PALETTE.ink }} formatter={(v) => formatMoneyRound(v)} />
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  {visibleSubBreakdown.map((s) => (
                    ...
                    >
                      ...
                      <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 13, width: 85, textAlign: "right", color: PALETTE.inkSoft }}>{formatMoneyRound(s.total)}</span>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ width: "100%", height: 200 }}>
                  <ResponsiveContainer>
                    <LineChart data={subTrend} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      <CartesianGrid stroke={PALETTE.rule} vertical={false} />
                      <XAxis dataKey="label" interval={0} tick={{ fontSize: 11, fill: PALETTE.inkSoft, fontFamily: "IBM Plex Sans" }} axisLine={{ stroke: PALETTE.rule }} tickLine={false} />
                      <YAxis domain={[subTrendTicks[0], subTrendTicks[subTrendTicks.length - 1]]} ticks={subTrendTicks} allowDecimals={false} tick={{ fontSize: 10, fill: PALETTE.inkSoft, fontFamily: "IBM Plex Mono" }} axisLine={false} tickLine={false} tickFormatter={(v) => formatMoneyAxis(v)} width={52} />
                      <Tooltip contentStyle={tooltipStyle} labelStyle={{ color: PALETTE.ink }} itemStyle={{ color: PALETTE.ink }} formatter={(v) => formatMoneyRound(v)} />
                      <Line type="monotone" dataKey="value" name="Importe" stroke={PALETTE.gold} strokeWidth={2} dot={{ r: 3, fill: PALETTE.gold }} />
                    </LineChart>
                  </ResponsiveContainer>
