...
function parseDecimal(input) {
  ...
  if (lastComma > -1 && lastDot > -1) {
    ...
    // only commas present: last comma is the decimal separator, earlier ones (if any) are thousands separators
...
// Para un activo marcado como "inversion", el coste (aportado neto) no es
// siempre el primer valor registrado: cada entrada posterior puede llevar un
// ajuste de aportacion/retirada (costBasisDelta) que sube o baja el capital
// aportado sin que eso cuente como ganancia o perdida de mercado. Esta
// funcion devuelve el historial con el coste ACUMULADO valido en cada punto.
function costBasisTimeline(history) {
  let basis = 0;
  return history.map((entry, i) => {
    if (i === 0) basis = entry.effectiveValue;
    else basis += (entry.costBasisDelta || 0);
    return { ...entry, costBasisAtPoint: basis };
  });
}
...
export default function App() {
  ...
  const [assetForm, setAssetForm] = useState({ name: "", value: "", date: todayISO(), category: "Cuenta Corriente", isInvestment: false, costBasisDelta: "", heldAmount: "" });
  const [editingAssetId, setEditingAssetId] = useState(null);
  const [assetHistoryFilter, setAssetHistoryFilter] = useState("");
  ...
  const [resumenYear, setResumenYear] = useState(CURRENT_YEAR);
  const [catYear, setCatYear] = useState(CURRENT_YEAR);
  ...
  async function loadNetWorthData() {
    try {
      ...
      setManualAssets((assetsRes.data || []).map((r) => {
        const value = Number(r.value);
        const heldAmount = r.held_amount != null ? Number(r.held_amount) : 0;
        return {
          id: r.id, name: r.name, value, heldAmount, effectiveValue: value + heldAmount, date: r.date,
          category: r.category || "Cuenta Corriente", isInvestment: !!r.is_investment,
          costBasisDelta: r.cost_basis_delta != null ? Number(r.cost_basis_delta) : 0,
        };
      }));
      setNetWorthError("");
    } catch (e) {
  ...
  async function addManualAsset() {
    const value = parseDecimal(assetForm.value);
    if (!assetForm.name.trim() || isNaN(value)) return;
    const rawDelta = String(assetForm.costBasisDelta || "").trim();
    const costBasisDelta = rawDelta === "" ? 0 : parseDecimal(rawDelta);
    if (isNaN(costBasisDelta)) { setNetWorthError("El importe de aportacion/retirada no es un numero valido."); return; }
    const rawHeld = String(assetForm.heldAmount || "").trim();
    const heldAmount = rawHeld === "" ? 0 : parseDecimal(rawHeld);
    if (isNaN(heldAmount)) { setNetWorthError("El importe retenido no es un numero valido."); return; }
    const asset = { name: assetForm.name.trim(), value, heldAmount, effectiveValue: value + heldAmount, date: assetForm.date || todayISO(), category: assetForm.category || "Cuenta Corriente", isInvestment: !!assetForm.isInvestment, costBasisDelta };
    try {
      if (editingAssetId) {
        const { error } = await supabase.from("manual_assets").update({ name: asset.name, value: asset.value, held_amount: asset.heldAmount, date: asset.date, category: asset.category, is_investment: asset.isInvestment, cost_basis_delta: asset.costBasisDelta }).eq("id", editingAssetId);
        if (error) throw error;
        setManualAssets(manualAssets.map((a) => (a.id === editingAssetId ? { ...asset, id: editingAssetId } : a)));
      } else {
        const id = uid();
        const { error } = await supabase.from("manual_assets").insert({ id, user_id: userId, name: asset.name, value: asset.value, held_amount: asset.heldAmount, date: asset.date, category: asset.category, is_investment: asset.isInvestment, cost_basis_delta: asset.costBasisDelta });
        if (error) throw error;
        setManualAssets([...manualAssets, { ...asset, id }]);
      }
      setAssetForm({ name: "", value: "", date: todayISO(), category: "Cuenta Corriente", isInvestment: false, costBasisDelta: "", heldAmount: "" });
      setEditingAssetId(null);
      setAddAssetModalOpen(false);
    } catch (e) {
      setNetWorthError(e.message || "No se pudo guardar el activo.");
    }
  }

  function openEditAssetEntry(entry) {
    ...
    setAssetForm({
      ...
      isInvestment: !!entry.isInvestment,
      costBasisDelta: entry.costBasisDelta ? String(entry.costBasisDelta) : "",
      heldAmount: entry.heldAmount ? String(entry.heldAmount) : "",
    });
    setAddAssetModalOpen(true);
  }

  async function removeManualAssetEntry(id) {
    try {
      const { error } = await supabase.from("manual_assets").delete().eq("id", id);
      if (error) throw error;
      setManualAssets(manualAssets.filter((a) => a.id !== id));
    } catch (e) {
      setNetWorthError(e.message || "No se pudo borrar el movimiento.");
    }
  }
  ...
  const sortedAssetHistory = useMemo(
    () => manualAssets.slice().sort((a, b) => (b.date || "").localeCompare(a.date || "") || a.name.localeCompare(b.name, "es")),
    [manualAssets]
  );
  const filteredAssetHistory = useMemo(() => {
    const q = assetHistoryFilter.trim().toLowerCase();
    if (!q) return sortedAssetHistory;
    return sortedAssetHistory.filter((a) => a.name.toLowerCase().includes(q));
  }, [sortedAssetHistory, assetHistoryFilter]);
  const manualTotal = visibleManualAssets.reduce((sum, a) => sum + a.effectiveValue, 0);

  // Historial de cada activo manual, agrupado por nombre y ordenado por
  // fecha. Sirve tanto para reconstruir el patrimonio mes a mes como para
  // calcular la ganancia/perdida de los que se han marcado como inversion
  // (comparando su ultimo valor con el primero registrado).
  ...
  // "Apuestas" cuenta como inversion (lleva coste/aportado neto igual que
  // cualquier otra), pero se trata como un bloque totalmente aparte del resto
  // de inversiones (otro tipo de riesgo, no queremos mezclar los totales).
  const manualInvestments = useMemo(() => {
    return visibleManualAssets
      .filter((a) => a.isInvestment)
      .map((a) => {
        const key = a.name.trim().toLowerCase();
        const history = manualHistoryByMonth[key] || [];
        const timeline = costBasisTimeline(history);
        const costBasis = timeline.length ? timeline[timeline.length - 1].costBasisAtPoint : a.effectiveValue;
        return { name: a.name, currentValue: a.effectiveValue, costBasis, gain: a.effectiveValue - costBasis, isBetting: a.category === "Apuestas" };
      });
  }, [visibleManualAssets, manualHistoryByMonth]);
  const manualRealInvestments = manualInvestments.filter((m) => !m.isBetting);
  const manualBettingInvestments = manualInvestments.filter((m) => m.isBetting);
  const manualInvestmentsGain = manualRealInvestments.reduce((sum, m) => sum + m.gain, 0);
  const manualBettingGain = manualBettingInvestments.reduce((sum, m) => sum + m.gain, 0);
  const manualInvestmentsTotal = visibleManualAssets.filter((a) => a.isInvestment && a.category !== "Apuestas").reduce((sum, a) => sum + a.effectiveValue, 0);
  const manualBettingTotal = visibleManualAssets.filter((a) => a.isInvestment && a.category === "Apuestas").reduce((sum, a) => sum + a.effectiveValue, 0);
  const manualAccountsTotal = manualTotal - manualInvestmentsTotal - manualBettingTotal;
  ...
  // Guardamos tambien el desglose por categoria para poder filtrar el
  // grafico por una o varias categorias sueltas (p.ej. solo "Apuestas").
  const reconstructedNetWorth = useMemo(() => {
    const months = new Set([
      ...Object.values(manualHistoryByMonth).flatMap((list) => list.map((a) => a.date.slice(0, 7))),
      currentMonthKey,
    ]);
    const sortedMonths = Array.from(months).filter((m) => m <= currentMonthKey).sort();
    return sortedMonths.map((month) => {
      ...
      Object.values(manualHistoryByMonth).forEach((list) => {
        ...
        if (latest) {
          total += latest.effectiveValue;
          byCategory[latest.category] = (byCategory[latest.category] || 0) + latest.effectiveValue;
        }
      });
      return { label: month, total, byCategory };
    });
  }, [manualHistoryByMonth, currentMonthKey]);

  // [] = "Todo". Con una o varias categorias seleccionadas, el grafico suma
  // solo esas categorias mes a mes.
  const [netWorthCategoryFilter, setNetWorthCategoryFilter] = useState([]);
  const netWorthAllCategories = useMemo(() => {
    const set = new Set(visibleManualAssets.map((a) => a.category));
    return Array.from(set).sort((a, b) => a.localeCompare(b, "es"));
  }, [visibleManualAssets]);
  function valueForNetWorthRow(row) {
    if (!netWorthCategoryFilter.length) return row.total;
    return netWorthCategoryFilter.reduce((sum, c) => sum + (row.byCategory[c] || 0), 0);
  }
  ...
  const netWorthRows = useMemo(() => {
    if (pnlYear === "all") {
      ...
      return pnlYearsAvailable.slice().sort().map((year) => {
        ...
        const val = monthsInYear.length ? valueForNetWorthRow(monthsInYear[monthsInYear.length - 1]) : prevVal;
        prevVal = val;
        return { label: year, value: val };
      });
    }
    let prevVal = lastKnownNetWorth(`${pnlYear}-00`);
    const rows = MONTHS_SHORT.map((m, i) => {
      ...
      const val = found ? valueForNetWorthRow(found) : prevVal;
      if (found) prevVal = val;
      return { label: m, value: val, monthKey };
    });
    // Nunca mostramos meses futuros: cortamos directamente en el mes actual
    // en vez de fiarnos de si hay "datos" (el valor arrastrado nunca es 0).
    const cappedRows = pnlYear === CURRENT_YEAR ? rows.filter((r) => r.monthKey <= currentMonthKey) : rows;
    return capMonths(cappedRows, pnlYear, (r) => r.value !== 0);
  }, [pnlYear, netWorthCategoryFilter, reconstructedNetWorth, pnlYearsAvailable, currentMonthKey]);
  ...
  // ese mes contra el coste (aportado neto) acumulado hasta ese mismo punto.
  function entryAtMonth(history, month) {
    let latest = null;
  ...
  // Usamos los mismos chips de categoria que el grafico de patrimonio para
  // decidir que activos entran en el beneficio/perdida: sin seleccion
  // ("Todo") se suman todas las inversiones EXCEPTO Apuestas (que es otro
  // tipo de riesgo y se contabiliza aparte); con una o varias categorias
  // seleccionadas, solo se cuentan esas (asi se puede ver, por ejemplo,
  // solo el beneficio/perdida de "Apuestas").
  const pnlInvestmentAssets = useMemo(() => {
    const investmentAssets = visibleManualAssets.filter((a) => a.isInvestment);
    if (!netWorthCategoryFilter.length) return investmentAssets.filter((a) => a.category !== "Apuestas");
    return investmentAssets.filter((a) => netWorthCategoryFilter.includes(a.category));
  }, [visibleManualAssets, netWorthCategoryFilter]);

  const allPnlMonthsSorted = useMemo(() => {
    const investmentMonths = pnlInvestmentAssets
  ...
  }, [currentMonthKey, pnlInvestmentAssets, manualHistoryByMonth]);

  const manualInvestmentGainByMonth = useMemo(() => {
    const map = {};
    pnlInvestmentAssets.forEach((a) => {
      const key = a.name.trim().toLowerCase();
      const history = manualHistoryByMonth[key] || [];
      if (history.length === 0) return;
      const timeline = costBasisTimeline(history);
      const startMonth = history[0].date.slice(0, 7);
      allPnlMonthsSorted.forEach((month) => {
        if (month < startMonth) return;
        const entry = entryAtMonth(timeline, month);
        if (entry == null) return;
        map[month] = (map[month] || 0) + (entry.effectiveValue - entry.costBasisAtPoint);
      });
    });
    return map;
  }, [pnlInvestmentAssets, manualHistoryByMonth, allPnlMonthsSorted]);
  ...
  // Degradado que hace que la MISMA linea/area se pinte de verde por encima
  // de cero y de rojo por debajo. El punto exacto del degradado donde ocurre
  // el cambio de color se calcula con los valores REALES de los datos (no
  // con los "ticks" redondeados del eje), porque el area SVG dibuja su caja
  // usando el rango real del trazo, y usar los ticks desplazaba el corte.
  const pnlCumulativeGradientOffset = useMemo(() => {
    const dataMax = Math.max(0, ...pnlRows.map((r) => r.cumulative));
    const dataMin = Math.min(0, ...pnlRows.map((r) => r.cumulative));
    if (dataMax <= 0) return 0;
    if (dataMin >= 0) return 1;
    return dataMax / (dataMax - dataMin);
  }, [pnlRows]);
  function renderPnlCumulativeDot(props) {
    const { cx, cy, payload, index } = props;
    const color = payload.cumulative >= 0 ? PALETTE.income : PALETTE.expense;
    return <circle key={`pnl-dot-${index}`} cx={cx} cy={cy} r={2} fill={color} stroke="none" />;
  }
  function renderPnlCumulativeActiveDot(props) {
    const { cx, cy, payload } = props;
    const color = payload.cumulative >= 0 ? PALETTE.income : PALETTE.expense;
    return <circle cx={cx} cy={cy} r={3} fill={color} stroke="none" />;
  }
  function renderPnlCumulativeTooltip({ active, payload, label }) {
    if (!active || !payload || !payload.length) return null;
    const value = payload[0].value;
    return (
      <div style={{ ...tooltipStyle, padding: "8px 10px" }}>
        <div style={{ color: PALETTE.inkSoft, marginBottom: 2 }}>{label}</div>
        <div style={{ color: value >= 0 ? PALETTE.income : PALETTE.expense, fontWeight: 600 }}>{formatMoney(value)}</div>
      </div>
    );
  }
  ...
  const subTrendAverage = useMemo(() => {
    const withData = subTrend.filter((r) => r.value > 0);
    if (!withData.length) return 0;
    return withData.reduce((s, r) => s + r.value, 0) / withData.length;
  }, [subTrend]);
                ...
                <div style={{ width: "100%", height: 200 }}>
                  <ResponsiveContainer>
                    <LineChart data={catTrend} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      ...
                      <Tooltip contentStyle={tooltipStyle} labelStyle={{ color: PALETTE.ink }} itemStyle={{ color: PALETTE.ink }} formatter={(v) => formatMoneyRound(v)} />
                      <Line type="monotone" dataKey="value" name="Importe" stroke={PALETTE.gold} strokeWidth={2} dot={{ r: 2, fill: PALETTE.gold }} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </section>
            ...
            {activeCatName && subSelected && (
              <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 28px", marginTop: 20 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
                  ...
                  {subTrendAverage > 0 && (
                    <span style={{ fontSize: 11, color: PALETTE.expense, background: PALETTE.paperDim, padding: "3px 8px", borderRadius: 10 }}>
                      Promedio {formatMoneyRound(subTrendAverage)}
                    </span>
                  )}
                </div>
                <div style={{ width: "100%", height: 200 }}>
                  <ResponsiveContainer>
                    <BarChart data={subTrend} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      <CartesianGrid stroke={PALETTE.rule} vertical={false} />
                      <XAxis dataKey="label" interval={0} tick={{ fontSize: 11, fill: PALETTE.inkSoft, fontFamily: FONT }} axisLine={{ stroke: PALETTE.rule }} tickLine={false} />
                      ...
                      <Tooltip contentStyle={tooltipStyle} labelStyle={{ color: PALETTE.ink }} itemStyle={{ color: PALETTE.ink }} formatter={(v) => formatMoneyRound(v)} />
                      <Bar dataKey="value" name="Importe" fill={PALETTE.expense} radius={[2, 2, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </section>
          ...
          <>
            <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 28px", marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 24, marginTop: 16, flexWrap: "wrap" }}>
                ...
                {manualRealInvestments.length > 0 && (
                  <div>
                ...
                {manualBettingInvestments.length > 0 && (
                  <div>
                    <p style={{ fontSize: 11, color: PALETTE.inkSoft, margin: "0 0 2px" }}>Apuestas</p>
                    <p style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 14, color: PALETTE.ink, margin: 0 }}>{formatMoneyRound(manualBettingTotal)}</p>
                  </div>
                )}
                {manualBettingInvestments.length > 0 && (
                  <div>
                    <p style={{ fontSize: 11, color: PALETTE.inkSoft, margin: "0 0 2px" }}>Beneficio / perdida (apuestas)</p>
                    <p style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 14, color: manualBettingGain >= 0 ? PALETTE.income : PALETTE.expense, margin: 0 }}>
                      {manualBettingGain >= 0 ? "+" : ""}{formatMoneyRound(manualBettingGain)}
                    </p>
                  </div>
                )}
              </div>
            ...
            <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "20px 24px", marginBottom: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                ...
                <button type="button" onClick={() => { setEditingAssetId(null); setAssetForm({ name: "", value: "", date: todayISO(), category: "Cuenta Corriente", isInvestment: false, costBasisDelta: "", heldAmount: "" }); setAddAssetModalOpen(true); }} style={{ display: "flex", alignItems: "center", gap: 6, border: `1px solid ${PALETTE.rule}`, borderRadius: 14, background: "none", color: PALETTE.gold, fontSize: 12, cursor: "pointer", padding: "4px 10px 4px 6px" }}>
              ...
              {!netWorthLoaded ? (
                ...
                visibleManualAssets.map((a) => {
                  const inv = a.isInvestment ? manualInvestments.find((m) => m.name === a.name) : null;
                  return (
                    <div key={a.name.toLowerCase()} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 0", borderBottom: `1px solid ${PALETTE.rule}` }}>
                      <div>
                        ...
                        {a.isInvestment && <span style={{ fontSize: 11, color: PALETTE.gold, marginLeft: 6 }}>· inversion</span>}
                        <div style={{ fontSize: 11, color: PALETTE.inkSoft, marginTop: 2 }}>
                          Actualizado el {formatDate(a.date)}
                          {inv && ` · aportado neto ${formatMoney(inv.costBasis)}`}
                          {!!a.heldAmount && ` · ${formatMoney(a.value)} liquidez + ${formatMoney(a.heldAmount)} retenido`}
                        </div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ textAlign: "right" }}>
                          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 14, color: PALETTE.ink }}>{formatMoney(a.effectiveValue)}</div>
                          {inv && (
                            <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 12, color: inv.gain >= 0 ? PALETTE.income : PALETTE.expense }}>
                              {inv.gain >= 0 ? "+" : ""}{formatMoney(inv.gain)}
                            </div>
                          )}
                        </div>
                        <button type="button" onClick={() => openEditAssetEntry(a)} aria-label="Editar" style={{ background: "none", border: "none", cursor: "pointer", color: PALETTE.gold, fontSize: 12 }}>editar</button>
            ...
            <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "20px 24px", marginBottom: 20 }}>
              <div
                onClick={() => setAssetHistoryOpen(!assetHistoryOpen)}
                style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", marginBottom: assetHistoryOpen ? 14 : 0 }}
              >
                <p style={{ fontSize: 12, color: PALETTE.inkSoft, margin: 0 }}>Historial de movimientos</p>
                <span style={{ fontSize: 12, color: PALETTE.inkSoft, transform: assetHistoryOpen ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.15s", display: "inline-block" }}>▾</span>
              </div>
              {assetHistoryOpen && (
                <>
                  <select
                    className="ledger-select"
                    value={assetHistoryFilter}
                    onChange={(e) => setAssetHistoryFilter(e.target.value)}
                    style={{ marginBottom: 14, maxWidth: 260 }}
                  >
                    <option value="">Todas las cuentas</option>
                    {visibleManualAssets.map((a) => <option key={a.name} value={a.name}>{a.name}</option>)}
                  </select>
                  {!netWorthLoaded ? (
                    <p style={{ fontSize: 13, color: PALETTE.inkSoft }}>Cargando...</p>
                  ) : filteredAssetHistory.length === 0 ? (
                    <p style={{ fontSize: 13, color: PALETTE.inkSoft, margin: 0 }}>
                      {assetHistoryFilter.trim() ? "No hay movimientos para esa cuenta." : "Todavia no hay movimientos registrados."}
                    </p>
                  ) : (
                    filteredAssetHistory.map((a) => (
                      <div key={a.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 0", borderBottom: `1px solid ${PALETTE.rule}`, gap: 10 }}>
                        <div>
                          <span style={{ fontSize: 14, color: PALETTE.ink }}>{a.name}</span>
                          <span style={{ fontSize: 11, color: PALETTE.inkSoft, background: PALETTE.paperDim, padding: "2px 7px", borderRadius: 3, marginLeft: 8 }}>{a.category}</span>
                          {a.isInvestment && <span style={{ fontSize: 11, color: PALETTE.gold, marginLeft: 6 }}>· inversion</span>}
                          <div style={{ fontSize: 11, color: PALETTE.inkSoft, marginTop: 2 }}>
                            Introducido el {formatDate(a.date)}
                            {!!a.costBasisDelta && ` · ${a.costBasisDelta > 0 ? "aportacion" : "retirada"} ${formatMoney(Math.abs(a.costBasisDelta))}`}
                            {!!a.heldAmount && ` · ${formatMoney(a.value)} liquidez + ${formatMoney(a.heldAmount)} retenido`}
                          </div>
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
                          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 14, color: PALETTE.ink }}>{formatMoney(a.effectiveValue)}</div>
                          <button type="button" onClick={() => openEditAssetEntry(a)} aria-label="Editar" style={{ background: "none", border: "none", cursor: "pointer", color: PALETTE.gold, fontSize: 12 }}>editar</button>
                          <button type="button" onClick={() => removeManualAssetEntry(a.id)} aria-label="Eliminar" style={{ background: "none", border: "none", cursor: "pointer", color: PALETTE.inkSoft, fontSize: 16 }}>x</button>
                        </div>
                      </div>
                    ))
                  )}
            ...
            {addAssetModalOpen && (
              <div onClick={() => { setAddAssetModalOpen(false); setEditingAssetId(null); }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 24 }}>
                <div onClick={(e) => e.stopPropagation()} style={{ background: PALETTE.paper, borderRadius: 6, padding: "22px 24px", maxWidth: 340, width: "100%" }}>
                  <p style={{ fontSize: 13, color: PALETTE.ink, margin: "0 0 4px", fontWeight: 500 }}>{editingAssetId ? "Editar movimiento" : "Nuevo valor de un activo manual"}</p>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setAssetForm({
                        ...assetForm,
                        name,
                        category: match ? match.category : assetForm.category,
                        isInvestment: match ? match.isInvestment : assetForm.isInvestment,
                      });
                    }}
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const investmentCategories = ["Acciones", "Fondo / ETF", "Apuestas"];
                      setAssetForm({
                        ...assetForm,
                        category: nextCategory,
                        isInvestment: investmentCategories.includes(nextCategory) ? true : assetForm.isInvestment,
                      });
                    }}
                    style={{ marginBottom: 12 }}
                  >
                    ...
                    <option value="Fondo / ETF">Fondo / ETF</option>
                  ...
                  {assetForm.category === "Apuestas" && (
                    <div style={{ marginBottom: 12 }}>
                      <label style={{ fontSize: 11, color: PALETTE.inkSoft }}>Importe retenido (apostado, sin resolver)</label>
                      <input
                        className="ledger-input"
                        inputMode="decimal"
                        placeholder="Ej. 500"
                        value={assetForm.heldAmount}
                        onChange={(e) => setAssetForm({ ...assetForm, heldAmount: e.target.value })}
                        style={{ marginBottom: 4 }}
                      />
                      <p style={{ fontSize: 11, color: PALETTE.inkSoft, margin: 0 }}>
                        Dinero que esta en apuestas todavia sin resolver (no en la cuenta como liquidez). Se suma al "Valor" para que no cuente como perdido en Patrimonio.
                      </p>
                    </div>
                  )}
                  <label style={{ fontSize: 11, color: PALETTE.inkSoft }}>Fecha de este valor</label>
                  <input className="ledger-input" type="date" value={assetForm.date} onChange={(e) => setAssetForm({ ...assetForm, date: e.target.value })} style={{ marginBottom: 14 }} />
                  <label style={{ display: "flex", alignItems: "flex-start", gap: 8, cursor: "pointer", background: PALETTE.paperDim, borderRadius: 8, padding: "10px 12px" }}>
                    ...
                    <span style={{ fontSize: 12, color: PALETTE.inkSoft }}>
                      Es una inversion (acciones, ETF, cripto, fondos...), no una cuenta/saldo. El primer valor que guardes se usa como aportado inicial, y cada actualizacion posterior se compara contra el aportado neto para calcular la ganancia o perdida mes a mes en Evolucion Patrimonio.
                    </span>
                  </label>
                  {assetForm.isInvestment && (
                    <div style={{ marginTop: 12 }}>
                      <label style={{ fontSize: 11, color: PALETTE.inkSoft }}>Aportacion o retirada en este movimiento (opcional)</label>
                      <input
                        className="ledger-input"
                        inputMode="decimal"
                        placeholder="Ej. 500 si metes dinero, -500 si sacas"
                        value={assetForm.costBasisDelta}
                        onChange={(e) => setAssetForm({ ...assetForm, costBasisDelta: e.target.value })}
                        style={{ marginBottom: 4 }}
                      />
                      <p style={{ fontSize: 11, color: PALETTE.inkSoft, margin: "0 0 12px" }}>
                        Dejalo en blanco si este nuevo valor es solo revalorizacion de mercado (ha subido o bajado sola). Ponlo si además metiste o sacaste dinero — así esa cantidad no cuenta como ganancia ni pérdida (no aplica a la primera vez que registras este activo).
                      </p>
                    </div>
                  )}
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 18 }}>
                    <button type="button" className="pill-btn" onClick={() => { setAddAssetModalOpen(false); setEditingAssetId(null); }}>Cancelar</button>
        ...
        {activeTab === "evolucion" && (
          <>
            <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 28px", marginBottom: 20 }}>
              ...
              {netWorthLoaded && netWorthTotal !== 0 && (
                <p style={{ fontSize: 12, color: PALETTE.inkSoft, margin: "6px 0 0" }}>
                  Liquidez {formatMoneyRound(manualAccountsTotal)} ({Math.round((manualAccountsTotal / netWorthTotal) * 100)}%)
                  {manualInvestmentsTotal !== 0 && (
                    <> · Invertido {formatMoneyRound(manualInvestmentsTotal)} ({Math.round((manualInvestmentsTotal / netWorthTotal) * 100)}%)</>
                  )}
                  {manualBettingTotal !== 0 && (
                    <> · Apuestas {formatMoneyRound(manualBettingTotal)} ({Math.round((manualBettingTotal / netWorthTotal) * 100)}%)</>
                  )}
                </p>
              )}
            </section>
            ...
            {netWorthChartData.length > 1 && (
              <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 28px", marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 4 }}>
                  <button
                    type="button"
                    className={"type-toggle" + (netWorthCategoryFilter.length === 0 ? " active-income" : "")}
                    style={{ borderRadius: 14, fontSize: 12, padding: "4px 10px" }}
                    onClick={() => setNetWorthCategoryFilter([])}
                  >
                    Todo
                  </button>
                  {netWorthAllCategories.map((cat) => (
                    <button
                      key={cat}
                      type="button"
                      className={"type-toggle" + (netWorthCategoryFilter.includes(cat) ? " active-income" : "")}
                      style={{ borderRadius: 14, fontSize: 12, padding: "4px 10px" }}
                      onClick={() => setNetWorthCategoryFilter((prev) => (prev.includes(cat) ? prev.filter((c) => c !== cat) : [...prev, cat]))}
                    >
                      {cat}
                    </button>
                  ))}
                ...
                <div style={{ width: "100%", height: 220 }}>
                  <ResponsiveContainer>
                    <AreaChart data={netWorthChartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      ...
                      <Area type="monotone" dataKey="value" name="Patrimonio" stroke={PALETTE.gold} strokeWidth={2} fill="url(#netWorthAreaFill)" dot={{ r: 2, fill: PALETTE.gold }} activeDot={{ r: 3 }} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              </section>
            ...
            <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 28px", marginBottom: 20 }}>
              ...
              <p style={{ fontSize: 11, color: PALETTE.inkSoft, margin: "0 0 4px" }}>
                Nunca vuelve a cero: es la suma de la ganancia/perdida mes a mes de{" "}
                {netWorthCategoryFilter.length ? netWorthCategoryFilter.join(", ") : "tus inversiones (sin contar Apuestas)"}.
              </p>
              <div style={{ marginBottom: 12 }} />
              {pnlRows.every((r) => r.cumulative === 0) ? (
                ...
                <div style={{ width: "100%", height: 220 }}>
                  <ResponsiveContainer>
                    <AreaChart data={pnlRows} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      <defs>
                        <linearGradient id="pnlCumulativeStroke" x1="0" y1="0" x2="0" y2="1">
                          <stop offset={pnlCumulativeGradientOffset} stopColor={PALETTE.income} />
                          <stop offset={pnlCumulativeGradientOffset} stopColor={PALETTE.expense} />
                        </linearGradient>
                        <linearGradient id="pnlCumulativeFill" x1="0" y1="0" x2="0" y2="1">
                          <stop offset={pnlCumulativeGradientOffset} stopColor={PALETTE.income} stopOpacity={0.35} />
                          <stop offset={pnlCumulativeGradientOffset} stopColor={PALETTE.expense} stopOpacity={0.35} />
                        </linearGradient>
                      </defs>
                      <CartesianGrid stroke={PALETTE.rule} vertical={false} />
                      <XAxis dataKey="label" interval={0} tick={{ fontSize: 11, fill: PALETTE.inkSoft, fontFamily: FONT }} axisLine={{ stroke: PALETTE.rule }} tickLine={false} />
                      ...
                      <Tooltip content={renderPnlCumulativeTooltip} />
                      <Area type="monotone" dataKey="cumulative" name="Acumulado" stroke="url(#pnlCumulativeStroke)" strokeWidth={2} fill="url(#pnlCumulativeFill)" dot={renderPnlCumulativeDot} activeDot={renderPnlCumulativeActiveDot} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              )}
