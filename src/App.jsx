...
export default function App() {
  ...
  async function computeHistoricalPnL(currentHoldings, closedPositions) {
    ...
    for (const h of withOpen) {
      ...
      if (priceSeries.length === 0) {
        // Twelve Data no tiene historico mensual para este simbolo (les pasa
        // sobre todo a ETFs europeos que solo pudimos localizar via Yahoo
        // Finance para el precio actual). Sin esto, los meses pasados de esta
        // posicion quedaban en blanco y el grafico parecia tener en cuenta
        // solo lo ya cerrado. Aproximamos su trayectoria interpolando
        // linealmente entre 0 (mes de apertura) y su ganancia/perdida actual
        // conocida hoy, para que el acumulado no pierda esta posicion.
        const openMonthFallback = h.openDate.slice(0, 7);
        if (openEur != null && priceMap[h.symbol] && openMonthFallback < nowMonth) {
          const currentGain = (priceMap[h.symbol].price - openEur) * h.quantity - (h.commission || 0);
          const months = monthsBetween(openMonthFallback, nowMonth);
    ...
    // desaparecer del acumulado solo porque hoy ya esten cerradas (si no,
    // el grafico daria un salto brusco justo en el mes de cierre, como si
    // solo existiera esa ultima posicion). Como ya no tenemos por que seguir
    // teniendo el precio/divisa de un simbolo que puede que ya ni se rastree,
    // aproximamos su evolucion interpolando linealmente entre 0 (mes de
    // apertura) y la ganancia/perdida final registrada al cerrarla (mes de
    // cierre, donde ya cuenta como realizada aparte, en pnlRealizedByMonth).
    (closedPositions || []).forEach((r) => {
      if (r.openDate == null || r.openPrice == null || !r.closeDate) return;
      const openMonth = r.openDate.slice(0, 7);
      const closeMonth = r.closeDate.slice(0, 7);
      if (openMonth >= closeMonth) return;
      const months = monthsBetween(openMonth, closeMonth);
      const span = months.length;
      months.forEach((month, idx) => {
        const fraction = (idx + 1) / (span + 1);
  ...
  async function addManualAsset() {
    ...
    const asset = { id: uid(), name: assetForm.name.trim(), value, date: assetForm.date || todayISO(), category: assetForm.category || "Cuenta Corriente", isInvestment: !!assetForm.isInvestment };
    try {
      const { error } = await supabase.from("manual_assets").insert({ id: asset.id, user_id: userId, name: asset.name, value: asset.value, date: asset.date, category: asset.category, is_investment: asset.isInvestment });
      if (error) throw error;
  ...
  const manualInvestments = useMemo(() => {
    return visibleManualAssets
      ...
      .map((a) => {
        ...
        const costBasis = history.length ? history[0].value : a.value;
  ...
  const manualInvestmentGainByMonth = useMemo(() => {
    ...
    visibleManualAssets.filter((a) => a.isInvestment).forEach((a) => {
      ...
      const costBasis = history[0].value;
      const startMonth = history[0].date.slice(0, 7);
      allPnlMonthsSorted.forEach((month) => {
        if (month < startMonth) return;
        const val = valueAtMonth(history, month);
        if (val == null) return;
        map[month] = (map[month] || 0) + (val - costBasis);
      });
    });
    return map;
                    ...
                    <div key={a.name.toLowerCase()} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 0", borderBottom: `1px solid ${PALETTE.rule}` }}>
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: PALETTE.inkSoft, marginTop: 2 }}>
                          ...
                          {inv && ` · coste inicial ${formatMoney(inv.costBasis)}`}
                        </div>
                      </div>
