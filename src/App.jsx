...
export default function App() {
  ...
  const [assetForm, setAssetForm] = useState({ name: "", value: "", date: todayISO(), category: "Cuenta Corriente", isInvestment: false, costBasisDelta: "" });
  ...
  async function loadNetWorthData() {
    try {
      ...
      setManualAssets((assetsRes.data || []).map((r) => ({ id: r.id, name: r.name, value: Number(r.value), date: r.date, category: r.category || "Cuenta Corriente", isInvestment: !!r.is_investment, costBasisDelta: r.cost_basis_delta != null ? Number(r.cost_basis_delta) : 0 })));
      setNetWorthError("");
    } catch (e) {
  ...
  async function addManualAsset() {
    ...
    const asset = { name: assetForm.name.trim(), value, date: assetForm.date || todayISO(), category: assetForm.category || "Cuenta Corriente", isInvestment: !!assetForm.isInvestment, costBasisDelta };
    try {
      if (editingAssetId) {
        const { error } = await supabase.from("manual_assets").update({ name: asset.name, value: asset.value, date: asset.date, category: asset.category, is_investment: asset.isInvestment, cost_basis_delta: asset.costBasisDelta }).eq("id", editingAssetId);
        if (error) throw error;
        setManualAssets(manualAssets.map((a) => (a.id === editingAssetId ? { ...asset, id: editingAssetId } : a)));
      } else {
        const id = uid();
        const { error } = await supabase.from("manual_assets").insert({ id, user_id: userId, name: asset.name, value: asset.value, date: asset.date, category: asset.category, is_investment: asset.isInvestment, cost_basis_delta: asset.costBasisDelta });
        if (error) throw error;
        setManualAssets([...manualAssets, { ...asset, id }]);
      }
      setAssetForm({ name: "", value: "", date: todayISO(), category: "Cuenta Corriente", isInvestment: false, costBasisDelta: "" });
  ...
  const manualInvestments = useMemo(() => {
    return visibleManualAssets
      ...
      .map((a) => {
        ...
        const costBasis = timeline.length ? timeline[timeline.length - 1].costBasisAtPoint : a.value;
        return { name: a.name, currentValue: a.value, costBasis, gain: a.value - costBasis, isBetting: a.category === "Apuestas" };
      });
  ...
  const manualInvestmentsTotal = visibleManualAssets.filter((a) => a.isInvestment && a.category !== "Apuestas").reduce((sum, a) => sum + a.value, 0);
  const manualBettingTotal = visibleManualAssets.filter((a) => a.isInvestment && a.category === "Apuestas").reduce((sum, a) => sum + a.value, 0);
  ...
  const reconstructedNetWorth = useMemo(() => {
    ...
    return sortedMonths.map((month) => {
      ...
      Object.values(manualHistoryByMonth).forEach((list) => {
        ...
        if (latest) {
          total += latest.value;
          byCategory[latest.category] = (byCategory[latest.category] || 0) + latest.value;
        }
      });
  ...
  const manualInvestmentGainByMonth = useMemo(() => {
    ...
    pnlInvestmentAssets.forEach((a) => {
      ...
      allPnlMonthsSorted.forEach((month) => {
        ...
        map[month] = (map[month] || 0) + (entry.value - entry.costBasisAtPoint);
      });
    });
    return map;
          ...
          <>
            ...
            <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "20px 24px", marginBottom: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                ...
                <button type="button" onClick={() => { setEditingAssetId(null); setAssetForm({ name: "", value: "", date: todayISO(), category: "Cuenta Corriente", isInvestment: false, costBasisDelta: "" }); setAddAssetModalOpen(true); }} style={{ display: "flex", alignItems: "center", gap: 6, border: `1px solid ${PALETTE.rule}`, borderRadius: 14, background: "none", color: PALETTE.gold, fontSize: 12, cursor: "pointer", padding: "4px 10px 4px 6px" }}>
                    ...
                    <div key={a.name.toLowerCase()} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 0", borderBottom: `1px solid ${PALETTE.rule}` }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ textAlign: "right" }}>
                          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 14, color: PALETTE.ink }}>{formatMoney(a.value)}</div>
                          {inv && (
                ...
                <div onClick={(e) => e.stopPropagation()} style={{ background: PALETTE.paper, borderRadius: 6, padding: "22px 24px", maxWidth: 340, width: "100%" }}>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const investmentCategories = ["Acciones", "Fondo / ETF"];
                      setAssetForm({
                        ...assetForm,
