...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await window.storage.get("app_users", true);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await window.storage.get("session_user", false);
  ...
  useEffect(() => {
    ...
    (async () => {
      setDataLoaded(false);
      try {
        const tRes = await window.storage.get(`udata_${currentUser}_transactions`, true);
        setTransactions(tRes && tRes.value ? JSON.parse(tRes.value) : []);
      } catch (e) {
        setTransactions([]);
      }
      try {
        const cRes = await window.storage.get(`udata_${currentUser}_categories`, true);
        if (cRes && cRes.value) {
          ...
          await window.storage.set(`udata_${currentUser}_categories`, JSON.stringify({ categories: DEFAULT_CATEGORIES, subcategories: DEFAULT_SUBCATEGORIES }), true);
        }
      } catch (e) {
        setCatConfig(DEFAULT_CATEGORIES);
        setSubConfig(DEFAULT_SUBCATEGORIES);
  ...
  async function persistTransactions(next) {
    ...
    try {
      const result = await window.storage.set(`udata_${currentUser}_transactions`, JSON.stringify(next), true);
  ...
  async function persistCategories(nextCats, nextSubs) {
    ...
    try {
      const result = await window.storage.set(`udata_${currentUser}_categories`, JSON.stringify({ categories: nextCats, subcategories: nextSubs }), true);
      setStorageError(!result);
    } catch (e) {
      setStorageError(true);
    }
  ...
  async function handleRegister(e) {
    ...
    try {
      ...
      const result = await window.storage.set("app_users", JSON.stringify(nextUsers), true);
      ...
      try { await window.storage.set("session_user", uname, false); } catch (e) { /* ignore */ }
    } catch (e) {
  ...
  function handleLogout() {
    ...
    window.storage.delete("session_user", false).catch(() => {});
  }
  ...
  const resumenRows = useMemo(() => {
    ...
    const capped = resumenYear === CURRENT_YEAR ? rows.slice(0, CURRENT_MONTH_INDEX + 1) : rows;
    return capped.map((r) => ({ ...r, savings: r.income - r.expense }));
  }, [transactions, resumenYear]);
  ...
  const catFiltered = useMemo(() => transactions.filter((t) => {
    if (t.type !== "expense") return false;
    if (catYear !== "all" && t.date.slice(0, 4) !== catYear) return false;
    return true;
  }), [transactions, catYear]);
  ...
  const catTrend = useMemo(() => {
    ...
    const relevant = transactions.filter((t) => t.type === "expense" && t.category === activeCatName);
    if (catYear === "all") {
      const map = {};
    ...
    return catYear === CURRENT_YEAR ? rows.slice(0, CURRENT_MONTH_INDEX + 1) : rows;
  }, [transactions, catYear, activeCatName]);
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "40px 24px 80px" }}>
        <header style={{ marginBottom: 20, display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
          ...
          <div style={{ textAlign: "right" }}>
            <p style={{ fontSize: 13, color: PALETTE.ink, margin: "0 0 4px", fontFamily: "'IBM Plex Sans', sans-serif" }}>{currentUser}</p>
            <button onClick={handleLogout} style={{ background: "none", border: "none", color: "rgba(242,244,245,0.72)", fontSize: 12, cursor: "pointer", padding: 0 }}>Cerrar sesion</button>
          </div>
        </header>
          ...
          <>
            ...
            <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 32px" }}>
              ...
              {!dataLoaded ? (
                ...
                <div>
                  {filtered.map((t) => (
                    <div key={t.id} className="ledger-row" style={{ display: "grid", gridTemplateColumns: "60px 1fr 150px 100px 28px", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: `1px solid ${PALETTE.rule}` }}>
                      <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 12, color: PALETTE.inkSoft }}>{formatDate(t.date)}</span>
                      <span style={{ fontSize: 14, color: PALETTE.ink }}>{t.description}</span>
                      <span style={{ fontSize: 11, color: t.type === "income" ? PALETTE.income : PALETTE.expense, background: t.type === "income" ? PALETTE.incomeBg : PALETTE.expenseBg, padding: "3px 8px", borderRadius: 3, justifySelf: "start" }}>
                        {t.category}{t.subcategory ? ` \u203A ${t.subcategory}` : ""}
                      </span>
                      <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 14, fontWeight: 500, textAlign: "right", color: t.type === "income" ? PALETTE.income : PALETTE.expense }}>
                ...
                <div style={{ width: "100%", height: 260 }}>
                  <ResponsiveContainer>
                    <BarChart data={resumenRows} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      ...
                      <XAxis dataKey="label" tick={{ fontSize: 12, fill: PALETTE.inkSoft, fontFamily: "IBM Plex Sans" }} axisLine={{ stroke: PALETTE.rule }} tickLine={false} />
                      <YAxis tick={{ fontSize: 11, fill: PALETTE.inkSoft, fontFamily: "IBM Plex Mono" }} axisLine={false} tickLine={false} tickFormatter={(v) => formatMoneyShort(v)} width={70} />
        ...
        {activeTab === "categorias" && (
          <>
            <section style={{ display: "flex", justifyContent: "flex-end", marginBottom: 16 }}>
              <select className="pill-btn" style={{ cursor: "pointer", background: PALETTE.paper }} value={catYear} onChange={(e) => setCatYear(e.target.value)}>
                <option value="all">Todos los anos</option>
                {yearsAvailable.map((y) => <option key={y} value={y}>{y}</option>)}
              </select>
            </section>
            <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 32px", marginBottom: 20 }}>
              ...
              {catBreakdown.length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {catBreakdown.map((c) => (
                    <div key={c.category} className="cat-row" onClick={() => setCatSelected(c.category)} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <span className="cat-name" style={{ fontSize: 13, width: 100, color: activeCatName === c.category ? PALETTE.gold : PALETTE.ink, fontWeight: activeCatName === c.category ? 500 : 400, flexShrink: 0 }}>{c.category}</span>
            ...
            {activeCatName && subBreakdown.length > 0 && (
              <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 28px", marginTop: 20 }}>
                <p style={{ fontSize: 12, color: PALETTE.inkSoft, margin: "0 0 16px" }}>
                  Desglose de {activeCatName.toLowerCase()} por subcategoria
                </p>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {subBreakdown.map((s) => (
                    <div key={s.subcategory} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <span style={{ fontSize: 13, width: 140, color: PALETTE.ink, flexShrink: 0 }}>{s.subcategory}</span>
                      <div style={{ flex: 1, height: 8, background: PALETTE.paperDim, borderRadius: 2, overflow: "hidden" }}>
                        <div style={{ width: `${maxSub ? (s.total / maxSub) * 100 : 0}%`, height: "100%", background: PALETTE.gold }} />
                      </div>
