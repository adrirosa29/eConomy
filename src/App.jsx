...
export default function App() {
  ...
  const realizedByMonth = useMemo(() => {
    ...
    return Object.entries(map).sort((a, b) => (a[0] < b[0] ? -1 : 1)).map(([key, gain]) => ({
      label: key.slice(2).replace("-", "/"), gain,
    }));
  }, [realizedGains]);
  const realizedByYear = useMemo(() => {
    ...
    return Object.entries(map).sort((a, b) => (a[0] < b[0] ? -1 : 1)).map(([label, gain]) => ({ label, gain }));
  }, [realizedGains]);
  const realizedRows = realizedView === "ano" ? realizedByYear : realizedByMonth;
  const realizedTicks = niceTicks(
    realizedRows.reduce((m, r) => Math.min(m, r.gain), 0),
    realizedRows.reduce((m, r) => Math.max(m, r.gain), 0),
    6
  );
              ...
              <>
                ...
                <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 28px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 10 }}>
                    <p style={{ fontSize: 12, color: PALETTE.inkSoft, margin: 0 }}>Beneficios / perdidas realizadas</p>
                    <div style={{ display: "flex" }}>
                  ...
                  {realizedRows.length === 0 ? (
                    <p style={{ fontSize: 13, color: PALETTE.inkSoft, margin: 0 }}>Cuando cierres una posicion, aqui vera su evolucion agregada.</p>
                  ) : (
                    <div style={{ width: "100%", height: 240 }}>
                      <ResponsiveContainer>
                        <BarChart data={realizedRows} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                          ...
                          <Tooltip contentStyle={tooltipStyle} labelStyle={{ color: PALETTE.ink }} itemStyle={{ color: PALETTE.ink }} formatter={(v) => formatMoney(v)} />
                          <Bar dataKey="gain" name="Beneficio/perdida" radius={[2, 2, 0, 0]}>
                            {realizedRows.map((r, i) => <Cell key={i} fill={r.gain >= 0 ? PALETTE.income : PALETTE.expense} />)}
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
