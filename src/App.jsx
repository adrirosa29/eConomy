...
export default function App() {
  ...
  async function addTransaction(e) {
    ...
    const ok = await persistTransactions(next);
    if (ok) {
      setAmount(""); setDesc(""); setSubcategory("");
    } else {
      setFormError("No se pudo guardar en la base de datos. Revisa tu conexion e intentalo de nuevo.");
    }
  }

  async function removeTransaction(id) {
    const ok = await persistTransactions(transactions.filter((t) => t.id !== id));
    if (!ok) {
      setFormError("No se pudo borrar el movimiento. Intentalo de nuevo.");
    }
  }
  ...
  async function handleImportFile(e) {
    ...
    try {
      ...
      if (ok) {
        ...
        setImportMsg(`Fallo al guardar en la base de datos. NO se ha importado nada (los ${added} movimientos no se guardaron). Intentalo de nuevo.`);
      }
    } catch (err) {
  ...
  async function reassignCategory(kind) {
    ...
    setReassignMsg({
      ...reassignMsg,
      [kind]: ok ? `Movidos ${count} movimientos de "${from}" a "${to}".` : `Fallo al mover los movimientos. Intentalo de nuevo.`,
    });
    if (ok) {
  ...
  async function reassignSubcategory() {
    ...
    const ok = await persistTransactions(next);
    setSubReassignMsg(ok ? `Movidos ${count} movimientos de "${subReassignFrom}" a "${subReassignTo}".` : "Fallo al mover los movimientos. Intentalo de nuevo.");
    if (ok) {
      setSubReassignFrom("");
      setSubReassignTo("");
    }
