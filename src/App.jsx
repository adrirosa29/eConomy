...
export default function App() {
  ...
  const realizedByMonth = useMemo(() => {
    ...
    if (!(currentMonthKey in map)) map[currentMonthKey] = 0;
    return Object.entries(map).sort((a, b) => (a[0] < b[0] ? -1 : 1)).map(([key, gain]) => ({
      label: key.slice(2).replace("-", "/"),
      total: gain + (key === currentMonthKey ? totalGain : 0),
    }));
  }, [realizedGains, currentMonthKey, totalGain]);
  const realizedByYear = useMemo(() => {
    ...
    if (!(CURRENT_YEAR in map)) map[CURRENT_YEAR] = 0;
    return Object.entries(map).sort((a, b) => (a[0] < b[0] ? -1 : 1)).map(([label, gain]) => ({
      label,
      total: gain + (label === CURRENT_YEAR ? totalGain : 0),
    }));
              ...
              <>
                ...
                <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 28px" }}>
                  ...
                  <p style={{ fontSize: 11, color: PALETTE.inkSoft, margin: "0 0 12px" }}>
                    Cada barra suma lo ya realizado (posiciones cerradas) mas, en el periodo actual, el beneficio/perdida sin realizar de tus posiciones abiertas ahora mismo.
                  </p>
