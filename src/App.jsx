...
export default function App() {
  ...
  // Punto (0-1) donde el area cruza el cero, para pintar la mitad positiva
  // en dorado y la mitad negativa en rojo dentro del mismo degradado.
  const pnlCumulativeZeroOffset = useMemo(() => {
    const dataMax = pnlCumulativeTicks[pnlCumulativeTicks.length - 1];
    const dataMin = pnlCumulativeTicks[0];
                    ...
                    <AreaChart data={pnlRows} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      <defs>
                        <linearGradient id="pnlCumulativeStroke" x1="0" y1="0" x2="0" y2="1">
                          <stop offset={pnlCumulativeZeroOffset} stopColor={PALETTE.gold} />
                          <stop offset={pnlCumulativeZeroOffset} stopColor={PALETTE.expense} />
                        </linearGradient>
                        <linearGradient id="pnlCumulativeFill" x1="0" y1="0" x2="0" y2="1">
                          <stop offset={pnlCumulativeZeroOffset} stopColor={PALETTE.gold} stopOpacity={0.35} />
                          <stop offset={pnlCumulativeZeroOffset} stopColor={PALETTE.expense} stopOpacity={0.35} />
                        </linearGradient>
                      </defs>
                      <CartesianGrid stroke={PALETTE.rule} vertical={false} />
                      <XAxis dataKey="label" interval={0} tick={{ fontSize: 11, fill: PALETTE.inkSoft, fontFamily: FONT }} axisLine={{ stroke: PALETTE.rule }} tickLine={false} />
                      ...
                      <Tooltip contentStyle={tooltipStyle} labelStyle={{ color: PALETTE.ink }} itemStyle={{ color: PALETTE.ink }} formatter={(v) => formatMoney(v)} />
                      <Area type="monotone" dataKey="cumulative" name="Acumulado" stroke="url(#pnlCumulativeStroke)" strokeWidth={2} fill="url(#pnlCumulativeFill)" dot={{ r: 2, fill: PALETTE.gold }} activeDot={{ r: 3 }} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              )}
