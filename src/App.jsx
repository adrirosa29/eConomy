...
export default function App() {
  ...
  async function loadNetWorthData() {
    try {
      const [assetsRes, holdingsRes, historyRes] = await Promise.all([
  ...
  async function addHolding() {
    ...
    const holding = {
      id: uid(), symbol, quantity, kind: holdingForm.kind,
    ...
    try {
      ...
      setNetWorthError(e.message || "No se pudo anadir la posicion. Revisa que el simbolo sea correcto (ej. AAPL, BTC/USD).");
    }
  }
  ...
  const holdingsWithGain = holdings.filter((h) => h.openPrice != null && !isNaN(h.openPrice) && priceMap[h.symbol] != null);
  const totalGain = holdingsWithGain.reduce((sum, h) => sum + (priceMap[h.symbol].price - h.openPrice) * h.quantity - (h.commission || 0), 0);
  ...
  async function handleLogout() {
    ...
    setManualAssets([]); setHoldings([]); setNetWorthHistory([]); setPriceMap({}); setNetWorthLoaded(false);
  }
          ...
          <>
            <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 28px", marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 24, marginTop: 16 }}>
                ...
                {holdingsWithGain.length > 0 && (
                  <div>
                    <p style={{ fontSize: 11, color: PALETTE.inkSoft, margin: "0 0 2px" }}>Beneficio / perdida</p>
            ...
            <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "20px 24px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
                <p style={{ fontSize: 12, color: PALETTE.inkSoft, margin: 0 }}>Acciones, ETF y cripto</p>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  {holdings.length > 0 && (
                  ...
                  <button type="button" onClick={() => setAddHoldingModalOpen(true)} style={{ display: "flex", alignItems: "center", gap: 6, border: `1px solid ${PALETTE.rule}`, borderRadius: 14, background: "none", color: PALETTE.gold, fontSize: 12, cursor: "pointer", padding: "4px 10px 4px 6px" }}>
                    <span style={{ width: 18, height: 18, borderRadius: "50%", border: `1px solid ${PALETTE.gold}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, lineHeight: 1 }}>+</span>
                    Anadir posicion
                  </button>
                </div>
              ...
              {!netWorthLoaded ? (
                ...
                holdings.map((h) => {
                  ...
                  const hasOpen = h.openPrice != null && !isNaN(h.openPrice);

                  // Modo "Acumulado": ganancia/perdida desde que se abrio la posicion (con comision restada)
                  const accGain = hasOpen && price != null ? (price - h.openPrice) * h.quantity - (h.commission || 0) : null;
                  const accGainPct = hasOpen && h.openPrice > 0 ? ((price - h.openPrice) / h.openPrice) * 100 : null;
                  ...
                  return (
                    <div key={h.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 0", borderBottom: `1px solid ${PALETTE.rule}` }}>
                      <div>
                        ...
                        {hasOpen && (
                          <div style={{ fontSize: 11, color: PALETTE.inkSoft, marginTop: 2 }}>
                            Abierta a {formatMoney(h.openPrice)}{h.openDate ? ` el ${formatDate(h.openDate)}` : ""}{h.commission ? ` · comision ${formatMoney(h.commission)}` : ""}
                          </div>
                        )}
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {addHoldingModalOpen && (
              <div onClick={() => setAddHoldingModalOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 24 }}>
                <div onClick={(e) => e.stopPropagation()} style={{ background: PALETTE.paper, borderRadius: 6, padding: "22px 24px", maxWidth: 340, width: "100%" }}>
                  <p style={{ fontSize: 13, color: PALETTE.ink, margin: "0 0 4px", fontWeight: 500 }}>Nueva posicion</p>
                  ...
                  <input className="ledger-input" placeholder="Ej. 10 o 0,5" value={holdingForm.quantity} onChange={(e) => setHoldingForm({ ...holdingForm, quantity: e.target.value })} style={{ marginBottom: 12 }} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addHolding(); } }} />
                  ...
                  <input className="ledger-input" placeholder="0,00" value={holdingForm.commission} onChange={(e) => setHoldingForm({ ...holdingForm, commission: e.target.value })} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addHolding(); } }} />
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 18 }}>
                    <button type="button" className="pill-btn" onClick={() => setAddHoldingModalOpen(false)}>Cancelar</button>
                    <button type="button" className="submit-btn" onClick={addHolding}>Anadir</button>
                  </div>
                </div>
              </div>
            )}
