...
export default function App() {
  ...
  }, [realizedGains, historicalPnL]);

  const realizedRows = useMemo(() => {
    if (pnlYear === "all") {
      const realizedByYear = {};
      Object.entries(pnlRealizedByMonth).forEach(([month, gain]) => {
        const y = month.slice(0, 4);
        realizedByYear[y] = (realizedByYear[y] || 0) + gain;
      });
      const lastUnrealizedByYear = {};
      Object.entries(historicalPnL).forEach(([month, gain]) => {
        const y = month.slice(0, 4);
        if (!lastUnrealizedByYear[y] || month > lastUnrealizedByYear[y].month) {
          lastUnrealizedByYear[y] = { month, gain };
        }
      });
      return pnlYearsAvailable.slice().sort().map((year) => {
        const realized = realizedByYear[year] || 0;
        const unrealized = year === CURRENT_YEAR ? totalGain : (lastUnrealizedByYear[year] ? lastUnrealizedByYear[year].gain : 0);
        return { label: year, total: realized + unrealized };
      });
    }
    const rows = MONTHS_SHORT.map((m, i) => {
      const monthKey = `${pnlYear}-${String(i + 1).padStart(2, "0")}`;
      const realized = pnlRealizedByMonth[monthKey] || 0;
      const unrealized = monthKey === currentMonthKey ? totalGain : (historicalPnL[monthKey] || 0);
      return { label: m, total: realized + unrealized };
    });
    return capMonths(rows, pnlYear, (r) => r.total !== 0);
  }, [pnlYear, pnlRealizedByMonth, historicalPnL, pnlYearsAvailable, currentMonthKey, totalGain]);
