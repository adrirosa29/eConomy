...
export default function App() {
  ...
  const [holdingForm, setHoldingForm] = useState({ symbol: "", quantity: "", kind: "stock", openPrice: "", openDate: todayISO(), commission: "" });
  ...
  async function loadNetWorthData() {
    try {
      ...
      setManualAssets((assetsRes.data || []).map((r) => ({ id: r.id, name: r.name, value: Number(r.value) })));
  ...
  async function fetchPrices(symbols) {
    ...
    try {
      ...
      if (symbols.length === 1) {
        ...
        symbols.forEach((s) => {
          const parsed = parseEntry(data[s]);
  ...
  async function refreshNetWorth(currentHoldings, currentAssets) {
    ...
    try {
      ...
      const manualTotal = currentAssets.reduce((sum, a) => sum + a.value, 0);
  ...
  async function addManualAsset() {
    ...
    const asset = { id: uid(), name: assetForm.name.trim(), value };
    try {
      const { error } = await supabase.from("manual_assets").insert({ id: asset.id, user_id: userId, name: asset.name, value: asset.value });
      if (error) throw error;
  ...
  async function removeManualAsset(id) {
    try {
      ...
      if (error) throw error;
      const nextAssets = manualAssets.filter((a) => a.id !== id);
      setManualAssets(nextAssets);
      refreshNetWorth(holdings, nextAssets);
    } catch (e) {
  ...
  async function saveHolding() {
    ...
    try {
      ...
      setHoldingForm({ symbol: "", quantity: "", kind: "stock", openPrice: "", openDate: todayISO(), commission: "" });
  ...
  useEffect(() => {
    if (activeTab !== "patrimonio" || !userId || netWorthLoaded) return;
    (async () => {
  ...
  async function loadHistoricalPnL() {
    ...
    try {
      const map = await computeHistoricalPnL(holdings);
      setHistoricalPnL(map);
    } catch (e) {
  ...
  useEffect(() => {
    if (patrimonioView !== "graficos" || !netWorthLoaded || pricesLoading || historicalLoaded || historicalLoading) return;
    loadHistoricalPnL();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [patrimonioView, netWorthLoaded, pricesLoading]);

  const manualTotal = manualAssets.reduce((sum, a) => sum + a.value, 0);
  ...
  const [realizedView, setRealizedView] = useState("mes");
  ...
  const realizedByMonth = useMemo(() => {
    ...
    realizedGains.forEach((r) => {
      ...
      realizedMap[key] = (realizedMap[key] || 0) + r.gain;
    });
    const months = new Set([...Object.keys(realizedMap), ...Object.keys(historicalPnL), currentMonthKey]);
    return Array.from(months).sort().map((key) => {
      const realized = realizedMap[key] || 0;
      const unrealized = key === currentMonthKey ? totalGain : (historicalPnL[key] || 0);
      return { label: key.slice(2).replace("-", "/"), total: realized + unrealized };
    });
  }, [realizedGains, historicalPnL, currentMonthKey, totalGain]);
  const realizedByYear = useMemo(() => {
    const realizedMap = {};
    realizedGains.forEach((r) => {
      const key = r.closeDate.slice(0, 4);
      realizedMap[key] = (realizedMap[key] || 0) + r.gain;
    });
    const unrealizedByYear = {};
    Object.entries(historicalPnL).forEach(([month, gain]) => {
      const year = month.slice(0, 4);
      // Nos quedamos con el ultimo mes conocido de cada ano (foto de cierre de ano)
      if (!unrealizedByYear[year] || month > unrealizedByYear[year].month) {
        unrealizedByYear[year] = { month, gain };
      }
    });
    const years = new Set([...Object.keys(realizedMap), ...Object.keys(unrealizedByYear), CURRENT_YEAR]);
    return Array.from(years).sort().map((year) => {
      const realized = realizedMap[year] || 0;
      const unrealized = year === CURRENT_YEAR ? totalGain : (unrealizedByYear[year] ? unrealizedByYear[year].gain : 0);
      return { label: year, total: realized + unrealized };
    });
  }, [realizedGains, historicalPnL, totalGain]);
  const realizedRows = realizedView === "ano" ? realizedByYear : realizedByMonth;
  const realizedTicks = niceTicks(
    realizedRows.reduce((m, r) => Math.min(m, r.total), 0),
    realizedRows.reduce((m, r) => Math.max(m, r.total), 0),
    6
  );
  ...
  async function handleLogout() {
    ...
    setHistoricalPnL({}); setHistoricalLoaded(false);
  }
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        {activeTab === "patrimonio" && (
          <>
            ...
            <nav style={{ display: "flex", gap: 20, borderBottom: `1px solid ${PALETTE.rule}`, marginBottom: 20 }}>
              <button type="button" className={"tab-btn" + (patrimonioView === "detalle" ? " active" : "")} onClick={() => setPatrimonioView("detalle")}>Detalle</button>
              <button type="button" className={"tab-btn" + (patrimonioView === "graficos" ? " active" : "")} onClick={() => setPatrimonioView("graficos")}>Graficos</button>
            </nav>

            {patrimonioView === "detalle" && (
              <>
                <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "20px 24px", marginBottom: 20 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                    ...
                    <button type="button" onClick={() => setAddAssetModalOpen(true)} style={{ display: "flex", alignItems: "center", gap: 6, border: `1px solid ${PALETTE.rule}`, borderRadius: 14, background: "none", color: PALETTE.gold, fontSize: 12, cursor: "pointer", padding: "4px 10px 4px 6px" }}>
                  ...
                  {!netWorthLoaded ? (
                    ...
                    manualAssets.map((a) => (
                      <div key={a.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 0", borderBottom: `1px solid ${PALETTE.rule}` }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          ...
                          <button onClick={() => removeManualAsset(a.id)} aria-label="Eliminar" style={{ background: "none", border: "none", cursor: "pointer", color: PALETTE.inkSoft, fontSize: 16 }}>x</button>
                        </div>
                      </div>
                    ))
                  )}
                ...
                <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "20px 24px", marginBottom: 20 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
                    <p style={{ fontSize: 12, color: PALETTE.inkSoft, margin: 0 }}>Acciones, ETF y cripto (abiertas)</p>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      {holdings.length > 0 && (
                      ...
                      <button
                        type="button"
                        onClick={() => { setEditingHoldingId(null); setHoldingForm({ symbol: "", quantity: "", kind: "stock", openPrice: "", openDate: todayISO(), commission: "" }); setAddHoldingModalOpen(true); }}
                  ...
                  {!netWorthLoaded ? (
                    ...
                    holdings.map((h) => {
                      ...
                      const openEur = openPriceEur[h.id];
                      const hasOpen = hasRawOpen && openEur != null;

                      // Modo "Acumulado": ganancia/perdida desde que se abrio la posicion,
                      // convirtiendo el precio de apertura a EUR con el cambio de ese dia
                      // (con comision restada)
                      const accGain = hasOpen && price != null ? (price - openEur) * h.quantity - (h.commission || 0) : null;
                      const accGainPct = hasOpen && openEur > 0 ? ((price - openEur) / openEur) * 100 : null;

                      // Modo "Hoy": variacion del dia segun el proveedor de precios
                      const dayGain = quote && quote.dayChange != null ? quote.dayChange * h.quantity : null;
                      ...
                      return (
                        <div key={h.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 0", borderBottom: `1px solid ${PALETTE.rule}`, gap: 10 }}>
                          <div>
                            <span style={{ fontSize: 14, color: PALETTE.ink }}>{h.symbol}</span>
                            <span style={{ fontSize: 12, color: PALETTE.inkSoft, marginLeft: 8 }}>{h.quantity} uds</span>
                            {hasRawOpen && (
                              <div style={{ fontSize: 11, color: PALETTE.inkSoft, marginTop: 2 }}>
                                ...
                                {hasRawOpen && !hasOpen && " · cambio no disponible"}
                              </div>
                            )}
                          </div>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                {realizedGains.length > 0 && (
                  <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "20px 24px" }}>
                    <p style={{ fontSize: 12, color: PALETTE.inkSoft, margin: "0 0 14px" }}>Posiciones cerradas</p>
            ...
            {patrimonioView === "graficos" && (
              <>
                {netWorthHistory.length > 1 && (
                  <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 28px", marginBottom: 20 }}>
                    <p style={{ fontSize: 12, color: PALETTE.inkSoft, margin: "0 0 12px" }}>Evolucion del patrimonio</p>
                    <div style={{ width: "100%", height: 220 }}>
                      <ResponsiveContainer>
                        <LineChart data={netWorthHistory.map((r) => ({ label: r.date.slice(5), value: r.total }))} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                          <CartesianGrid stroke={PALETTE.rule} vertical={false} />
                          <XAxis dataKey="label" tick={{ fontSize: 10, fill: PALETTE.inkSoft, fontFamily: FONT }} axisLine={{ stroke: PALETTE.rule }} tickLine={false} />
                          <YAxis domain={[netWorthTicks[0], netWorthTicks[netWorthTicks.length - 1]]} ticks={netWorthTicks} allowDecimals={false} tick={{ fontSize: 10, fill: PALETTE.inkSoft, fontFamily: FONT }} axisLine={false} tickLine={false} tickFormatter={(v) => formatMoneyAxis(v)} width={52} />
                ...
                <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 28px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4, flexWrap: "wrap", gap: 10 }}>
                    <p style={{ fontSize: 12, color: PALETTE.inkSoft, margin: 0 }}>Beneficios / perdidas (realizadas + abiertas)</p>
                    <div style={{ display: "flex" }}>
                      <button type="button" className={"type-toggle" + (realizedView === "mes" ? " active-income" : "")} style={{ borderRadius: "3px 0 0 3px", fontSize: 12, padding: "5px 12px" }} onClick={() => setRealizedView("mes")}>Por mes</button>
                      <button type="button" className={"type-toggle" + (realizedView === "ano" ? " active-income" : "")} style={{ borderRadius: "0 3px 3px 0", borderLeft: "none", fontSize: 12, padding: "5px 12px" }} onClick={() => setRealizedView("ano")}>Por ano</button>
                    </div>
                  </div>
                  <p style={{ fontSize: 11, color: PALETTE.inkSoft, margin: "0 0 4px" }}>
                    Cada barra suma lo ya realizado (posiciones cerradas) mas la ganancia/perdida sin realizar que tenian tus posiciones abiertas en ese momento, reconstruida con precios historicos desde su fecha de apertura.
                  </p>
                  ...
                  {realizedRows.every((r) => r.total === 0) ? (
                    <p style={{ fontSize: 13, color: PALETTE.inkSoft, margin: 0 }}>Todavia no hay beneficios o perdidas que mostrar, ni realizadas ni en posiciones abiertas.</p>
                  ) : (
                    <div style={{ width: "100%", height: 240 }}>
                      <ResponsiveContainer>
                        <BarChart data={realizedRows} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                          <CartesianGrid stroke={PALETTE.rule} vertical={false} />
                          <XAxis dataKey="label" interval={0} tick={{ fontSize: 11, fill: PALETTE.inkSoft, fontFamily: FONT }} axisLine={{ stroke: PALETTE.rule }} tickLine={false} />
                          <YAxis domain={[realizedTicks[0], realizedTicks[realizedTicks.length - 1]]} ticks={realizedTicks} allowDecimals={false} tick={{ fontSize: 10, fill: PALETTE.inkSoft, fontFamily: FONT }} axisLine={false} tickLine={false} tickFormatter={(v) => formatMoneyAxis(v)} width={52} />
                          <Tooltip contentStyle={tooltipStyle} labelStyle={{ color: PALETTE.ink }} itemStyle={{ color: PALETTE.ink }} formatter={(v) => formatMoney(v)} />
                          <Bar dataKey="total" name="Beneficio/perdida" radius={[2, 2, 0, 0]}>
                            {realizedRows.map((r, i) => <Cell key={i} fill={r.total >= 0 ? PALETTE.income : PALETTE.expense} />)}
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
            ...
            {addAssetModalOpen && (
              <div onClick={() => setAddAssetModalOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 24 }}>
                <div onClick={(e) => e.stopPropagation()} style={{ background: PALETTE.paper, borderRadius: 6, padding: "22px 24px", maxWidth: 340, width: "100%" }}>
                  <p style={{ fontSize: 13, color: PALETTE.ink, margin: "0 0 14px", fontWeight: 500 }}>Nuevo activo manual</p>
                  <label style={{ fontSize: 11, color: PALETTE.inkSoft }}>Nombre</label>
                  <input className="ledger-input" autoFocus placeholder="Ej. Cuenta corriente" value={assetForm.name} onChange={(e) => setAssetForm({ ...assetForm, name: e.target.value })} style={{ marginBottom: 12 }} />
                  <label style={{ fontSize: 11, color: PALETTE.inkSoft }}>Valor (EUR)</label>
                  <input className="ledger-input" placeholder="0,00" value={assetForm.value} onChange={(e) => setAssetForm({ ...assetForm, value: e.target.value })} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addManualAsset(); } }} />
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 18 }}>
                    <button type="button" className="pill-btn" onClick={() => setAddAssetModalOpen(false)}>Cancelar</button>
                    <button type="button" className="submit-btn" onClick={addManualAsset}>Anadir</button>
                  </div>
                </div>
              </div>
            )}
            ...
            {addHoldingModalOpen && (
              <div onClick={() => { setAddHoldingModalOpen(false); setEditingHoldingId(null); }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 24 }}>
                <div onClick={(e) => e.stopPropagation()} style={{ background: PALETTE.paper, borderRadius: 6, padding: "22px 24px", maxWidth: 340, width: "100%" }}>
                  <p style={{ fontSize: 13, color: PALETTE.ink, margin: "0 0 4px", fontWeight: 500 }}>{editingHoldingId ? "Editar posicion" : "Nueva posicion"}</p>
                  <p style={{ fontSize: 12, color: PALETTE.inkSoft, margin: "0 0 14px" }}>Simbolo tal cual en el mercado: AAPL, MSFT, VWCE, BTC/USD...</p>
                  <label style={{ fontSize: 11, color: PALETTE.inkSoft }}>Simbolo</label>
                  <input className="ledger-input" autoFocus placeholder="Ej. AAPL o BTC/USD" value={holdingForm.symbol} onChange={(e) => setHoldingForm({ ...holdingForm, symbol: e.target.value })} style={{ marginBottom: 12 }} />
                  <label style={{ fontSize: 11, color: PALETTE.inkSoft }}>Cantidad</label>
                  <input className="ledger-input" placeholder="Ej. 10 o 0,5" value={holdingForm.quantity} onChange={(e) => setHoldingForm({ ...holdingForm, quantity: e.target.value })} style={{ marginBottom: 12 }} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); saveHolding(); } }} />
                  <label style={{ fontSize: 11, color: PALETTE.inkSoft }}>Tipo</label>
                  <select className="ledger-select" value={holdingForm.kind} onChange={(e) => setHoldingForm({ ...holdingForm, kind: e.target.value })} style={{ marginBottom: 12 }}>
                    <option value="stock">Accion</option>
                    <option value="etf">ETF / Fondo cotizado</option>
                    <option value="crypto">Criptomoneda</option>
                  </select>
                  <p style={{ fontSize: 11, color: PALETTE.inkSoft, margin: "4px 0 10px", borderTop: `1px solid ${PALETTE.rule}`, paddingTop: 10 }}>
                  ...
                  <input className="ledger-input" placeholder="Precio por unidad al comprar" value={holdingForm.openPrice} onChange={(e) => setHoldingForm({ ...holdingForm, openPrice: e.target.value })} style={{ marginBottom: 12 }} />
                  ...
                  <input className="ledger-input" placeholder="0,00" value={holdingForm.commission} onChange={(e) => setHoldingForm({ ...holdingForm, commission: e.target.value })} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); saveHolding(); } }} />
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 18 }}>
                    <button type="button" className="pill-btn" onClick={() => { setAddHoldingModalOpen(false); setEditingHoldingId(null); }}>Cancelar</button>
