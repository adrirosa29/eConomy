...
export default function App() {
  ...
  // Partimos el acumulado en dos series (positiva y negativa) para poder
  // pintar cada tramo del area en su color, sin depender de degradados por
  // posicion en pantalla (que no cuadran bien cuando el eje tiene margen).
  const pnlCumulativeSplitRows = useMemo(
    () => pnlRows.map((r) => ({
      ...r,
      cumulativePos: r.cumulative >= 0 ? r.cumulative : 0,
      cumulativeNeg: r.cumulative < 0 ? r.cumulative : 0,
    })),
    [pnlRows]
  );
  function renderPnlCumulativeTooltip({ active, payload, label }) {
    if (!active || !payload || !payload.length) return null;
    const point = payload.find((p) => p.value !== 0) || payload[0];
    return (
      <div style={{ ...tooltipStyle, padding: "8px 10px" }}>
        <div style={{ color: PALETTE.inkSoft, marginBottom: 2 }}>{label}</div>
        <div style={{ color: point.value >= 0 ? PALETTE.income : PALETTE.expense, fontWeight: 600 }}>{formatMoney(point.value)}</div>
      </div>
    );
  }
          ...
          <>
            ...
            <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 28px", marginBottom: 20 }}>
              ...
              {pnlRows.every((r) => r.cumulative === 0) ? (
                ...
                <div style={{ width: "100%", height: 220 }}>
                  <ResponsiveContainer>
                    <AreaChart data={pnlCumulativeSplitRows} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      <CartesianGrid stroke={PALETTE.rule} vertical={false} />
                      <XAxis dataKey="label" interval={0} tick={{ fontSize: 11, fill: PALETTE.inkSoft, fontFamily: FONT }} axisLine={{ stroke: PALETTE.rule }} tickLine={false} />
                      ...
                      <Area type="monotone" dataKey="cumulativePos" name="Acumulado" stroke={PALETTE.income} strokeWidth={2} fill={PALETTE.income} fillOpacity={0.35} dot={{ r: 2, fill: PALETTE.income }} activeDot={{ r: 3 }} />
                      <Area type="monotone" dataKey="cumulativeNeg" name="Acumulado" stroke={PALETTE.expense} strokeWidth={2} fill={PALETTE.expense} fillOpacity={0.35} dot={{ r: 2, fill: PALETTE.expense }} activeDot={{ r: 3 }} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              )}
