...
export default function App() {
    ...
    try {
      ...
      if (!data.values) {
        if (data.status === "error" || data.code) {
          setNetWorthError(`Twelve Data (${symbol}): ${data.message || "error desconocido al pedir historico"}`);
        }
        return [];
      }
      return data.values
      ...
      setNetWorthError(`Fallo de red pidiendo historico de ${symbol}: ${e.message}`);
      return [];
    }
  }
  ...
  const netWorthChartData = reconstructedNetWorth.length > 1
    ? reconstructedNetWorth.map((r) => ({ label: r.label.slice(2).replace("-", "/"), value: r[netWorthValueKey] }))
