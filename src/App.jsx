...
export default function App() {
  ...
  const [assetForm, setAssetForm] = useState({ name: "", value: "", date: todayISO() });
  ...
  async function loadNetWorthData() {
    try {
      ...
      setManualAssets((assetsRes.data || []).map((r) => ({ id: r.id, name: r.name, value: Number(r.value), date: r.date })));
  ...
  async function addManualAsset() {
    ...
    const asset = { id: uid(), name: assetForm.name.trim(), value, date: assetForm.date || todayISO() };
    try {
      const { error } = await supabase.from("manual_assets").insert({ id: asset.id, user_id: userId, name: asset.name, value: asset.value, date: asset.date });
      if (error) throw error;
      const nextAssets = [...manualAssets, asset];
      setManualAssets(nextAssets);
      setAssetForm({ name: "", value: "", date: todayISO() });
  ...
  const reconstructedNetWorth = useMemo(() => {
    ...
    return sortedMonths.map((month) => {
      ...
      return { label: month, total: manualTotalAtMonth + holdingsValueAtMonth };
    });
  }, [historicalValue, manualHistoryByMonth, currentMonthKey, holdingsTotal]);

  const netWorthChartData = reconstructedNetWorth.length > 1
    ? reconstructedNetWorth.map((r) => ({ label: r.label.slice(2).replace("-", "/"), value: r.total }))
  ...
  async function handleLogout() {
    ...
    setHistoricalPnL({}); setHistoricalValue({}); setHistoricalLoaded(false);
  }
          ...
          <>
            ...
            <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "20px 24px", marginBottom: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                ...
                <button type="button" onClick={() => { setAssetForm({ name: "", value: "", date: todayISO() }); setAddAssetModalOpen(true); }} style={{ display: "flex", alignItems: "center", gap: 6, border: `1px solid ${PALETTE.rule}`, borderRadius: 14, background: "none", color: PALETTE.gold, fontSize: 12, cursor: "pointer", padding: "4px 10px 4px 6px" }}>
            ...
            {addAssetModalOpen && (
              <div onClick={() => setAddAssetModalOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 24 }}>
                <div onClick={(e) => e.stopPropagation()} style={{ background: PALETTE.paper, borderRadius: 6, padding: "22px 24px", maxWidth: 340, width: "100%" }}>
                  ...
                  <input className="ledger-input" autoFocus list="manual-asset-names" placeholder="Ej. Cuenta corriente" value={assetForm.name} onChange={(e) => setAssetForm({ ...assetForm, name: e.target.value })} style={{ marginBottom: 12 }} />
        ...
        {activeTab === "evolucion" && (
          <>
            ...
            {netWorthChartData.length > 1 && (
              <section style={{ background: PALETTE.paper, borderRadius: 4, padding: "24px 28px", marginBottom: 20 }}>
                <p style={{ fontSize: 12, color: PALETTE.inkSoft, margin: "0 0 4px" }}>Evolucion del patrimonio</p>
                <p style={{ fontSize: 11, color: PALETTE.inkSoft, margin: "0 0 12px" }}>
                  Reconstruida con el historial de tus activos manuales y el valor de mercado mes a mes.
                </p>
                <div style={{ width: "100%", height: 220 }}>
                  <ResponsiveContainer>
