...
export default function App() {
  ...
  const [holdingForm, setHoldingForm] = useState({ symbol: "", quantity: "", kind: "stock" });
  ...
  async function loadNetWorthData() {
    try {
      ...
      setHoldings((holdingsRes.data || []).map((r) => ({ id: r.id, symbol: r.symbol, quantity: Number(r.quantity), kind: r.kind })));
  ...
  async function fetchPrices(symbols) {
    ...
    try {
      const res = await fetch(`https://api.twelvedata.com/price?symbol=${encodeURIComponent(symbols.join(","))}&apikey=${TWELVE_DATA_KEY}`);
      const data = await res.json();
      const map = {};
      if (symbols.length === 1) {
        const v = parseFloat(data.price);
        if (!isNaN(v)) map[symbols[0]] = v;
      } else {
        symbols.forEach((s) => {
          const entry = data[s];
          const v = entry && parseFloat(entry.price);
          if (!isNaN(v)) map[s] = v;
        });
      }
      return map;
  ...
  async function refreshNetWorth(currentHoldings, currentAssets) {
    ...
    try {
      ...
      setPriceMap(prices);
      const holdingsTotal = currentHoldings.reduce((sum, h) => sum + (prices[h.symbol] || 0) * h.quantity, 0);
  ...
  async function addHolding() {
    ...
    const holding = { id: uid(), symbol, quantity, kind: holdingForm.kind };
    try {
      const { error } = await supabase.from("holdings").insert({ id: holding.id, user_id: userId, symbol: holding.symbol, quantity: holding.quantity, kind: holding.kind });
      if (error) throw error;
      const nextHoldings = [...holdings, holding];
      setHoldings(nextHoldings);
      setHoldingForm({ symbol: "", quantity: "", kind: "stock" });
  ...
  const holdingsTotal = holdings.reduce((sum, h) => sum + (priceMap[h.symbol] || 0) * h.quantity, 0);
              ...
              {!netWorthLoaded ? (
                ...
                holdings.map((h) => (
                  <div key={h.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 0", borderBottom: `1px solid ${PALETTE.rule}` }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 14, color: PALETTE.ink }}>
                        {priceMap[h.symbol] != null ? formatMoney(priceMap[h.symbol] * h.quantity) : "..."}
                      </span>
