...
// Respaldo para simbolos que Twelve Data no cubre en el plan gratuito (ej.
// ETF en bolsas europeas). Solo se usa para el precio actual, no para el
// historico mensual, ya que Finnhub tampoco lo da gratis para esos mercados.
const FINNHUB_KEY = "dan6ne9r01qn0fqa1e50dan6ne9r01qn0fqa1e5g";
...
export default function App() {
  ...
  async function fetchFinnhubQuote(finnhubSymbol) {
    try {
      const res = await fetch(`https://finnhub.io/api/v1/quote?symbol=${encodeURIComponent(finnhubSymbol)}&token=${FINNHUB_KEY}`);
      const data = await res.json();
      if (data.c == null || data.c === 0) return null;
      return {
        price: data.c,
        dayChange: data.d != null ? data.d : null,
        dayPercentChange: data.dp != null ? data.dp : null,
        currency: "EUR",
      };
    } catch (e) {
      console.error("Error obteniendo precio de Finnhub:", e);
      return null;
    }
  }
  ...
  async function refreshNetWorth(currentHoldings, currentAssets) {
    ...
    try {
      ...
      // Respaldo: para simbolos que Twelve Data no pudo dar (ej. mercados
      // fuera de su plan gratuito), si la posicion tiene un simbolo
      // alternativo de Finnhub, probamos ahi.
      const symbolsMissing = symbols.filter((s) => !prices[s]);
      for (const s of symbolsMissing) {
        const holdingWithFallback = currentHoldings.find((h) => h.symbol === s && h.finnhubSymbol);
        if (!holdingWithFallback) continue;
        const finnhubPrice = await fetchFinnhubQuote(holdingWithFallback.finnhubSymbol);
        if (finnhubPrice) prices[s] = finnhubPrice;
      }
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: openPositionsExpanded ? 14 : 0, flexWrap: "wrap", gap: 10 }}>
                ...
                {openPositionsExpanded && (
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <button
                      type="button"
                      onClick={() => { setEditingHoldingId(null); setHoldingForm({ symbol: "", quantity: "", kind: "stock", openPrice: "", openDate: todayISO(), commission: "", finnhubSymbol: "" }); setSymbolPickedName(""); setSymbolResults([]); setAddHoldingModalOpen(true); }}
              ...
              <div onClick={() => { setAddHoldingModalOpen(false); setEditingHoldingId(null); }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 50 }}>
                ...
                >
                  ...
                  <div style={{ background: PALETTE.paperDim, borderRadius: 8, padding: "14px 14px 4px", marginBottom: 18 }}>
                    <p style={{ fontSize: 11, color: PALETTE.inkSoft, margin: "0 0 12px" }}>
                      Opcional — si Twelve Data no cubre este mercado (ej. bolsas europeas), prueba con el simbolo equivalente en Finnhub (ej. IUSN.DE). Solo se usa para el precio actual, no para el historico.
                    </p>
                    <label style={{ fontSize: 11, color: PALETTE.inkSoft }}>Simbolo alternativo (Finnhub)</label>
                    <input className="ledger-input" placeholder="Ej. IUSN.DE" value={holdingForm.finnhubSymbol} onChange={(e) => setHoldingForm({ ...holdingForm, finnhubSymbol: e.target.value })} style={{ marginBottom: 12 }} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); saveHolding(); } }} />
                  </div>
