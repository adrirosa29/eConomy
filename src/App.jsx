...
export default function App() {
  ...
  async function searchFinnhubSymbols(query) {
    if (!query || query.trim().length < 2) return [];
    try {
      const res = await fetch(`https://finnhub.io/api/v1/search?q=${encodeURIComponent(query.trim())}&token=${FINNHUB_KEY}`);
      const data = await res.json();
      return (data.result || []).slice(0, 8);
    } catch (e) {
      console.error("Error buscando en Finnhub:", e);
      return [];
    }
  }
  ...
  async function fetchFinnhubQuote(finnhubSymbol) {
    try {
      ...
      const data = await res.json();
      if (data.error) {
        setNetWorthError(`Finnhub (${finnhubSymbol}): ${data.error}`);
        return null;
      }
      if (data.c == null || data.c === 0) {
        setNetWorthError(`Finnhub no devolvio precio para "${finnhubSymbol}". Comprueba que el simbolo es exactamente el que aparece en su buscador.`);
        return null;
      }
      return {
        price: data.c,
      ...
      setNetWorthError(`Fallo de red pidiendo precio a Finnhub para "${finnhubSymbol}": ${e.message}`);
      return null;
    }
  }
  ...
  async function refreshNetWorth(currentHoldings, currentAssets) {
    ...
    setNetWorthError("");
    try {
      // Si el usuario ha puesto un simbolo alternativo de Finnhub para una
      // posicion, es una senal explicita de que Twelve Data no sirve para
      // ese valor — ni lo intentamos ahi, para evitar que un ticker
      ...
      // Para los simbolos con respaldo de Finnhub (o los que Twelve Data no
      // pudo dar), probamos ahi.
              ...
              <div onClick={() => { setAddHoldingModalOpen(false); setEditingHoldingId(null); }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 50 }}>
                ...
                >
                  ...
                  <div style={{ background: PALETTE.paperDim, borderRadius: 8, padding: "14px 14px 4px", marginBottom: 18 }}>
                    <p style={{ fontSize: 11, color: PALETTE.inkSoft, margin: "0 0 12px" }}>
                      Opcional — si Twelve Data no cubre este mercado (ej. bolsas europeas), busca aqui el equivalente en Finnhub. Solo se usa para el precio actual, no para el historico.
                    </p>
                    ...
                    <div style={{ position: "relative" }}>
                      ...
                      />
                      {finnhubSearching && (
                        <p style={{ fontSize: 11, color: PALETTE.inkSoft, margin: "-8px 0 8px" }}>Buscando...</p>
                      )}
                      {finnhubResults.length > 0 && (
                        <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: PALETTE.paper, border: `1px solid ${PALETTE.rule}`, borderRadius: 6, marginTop: -8, maxHeight: 220, overflowY: "auto", zIndex: 60, boxShadow: "0 6px 20px rgba(0,0,0,0.25)" }}>
                          {finnhubResults.map((r, i) => (
                            ...
                            >
                              <div style={{ fontSize: 13, color: PALETTE.ink }}>{r.symbol} <span style={{ color: PALETTE.inkSoft, fontWeight: 400 }}>— {r.description}</span></div>
                              {r.type && <div style={{ fontSize: 11, color: PALETTE.inkSoft, marginTop: 1 }}>{r.type}</div>}
                            </button>
                          ))}
                        </div>
                      )}
